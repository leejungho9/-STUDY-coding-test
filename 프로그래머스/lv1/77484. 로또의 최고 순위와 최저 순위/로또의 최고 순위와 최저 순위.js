function solution(lottos, win_nums) {
    ...
    let wins = lottos.filter(lotto => win_nums.includes(lotto)).length;
    let zero = lottos.filter(lotto => lotto === 0).length;
     
    let Max = 7 - (wins + zero);  //최대로 맞춘 등수 
    let Min = 7 - wins;         //최소로 맞춘 등수
    
    console.log(wins, zero)
    if(wins === 0 && zero === 0) { //맞춘 번호도 , 0도 없는 경우
        Max = 6;
        Min = 6;
    }else if(wins === 0) {  //맞춘 번호가 없는 경우 
        Min = 6;
    }
